...
async function loadMarkdown(filePath) {
  ...
  // Split on headers BEFORE converting to HTML, since stripHtml() collapses
  // header tags and would otherwise merge every section into one blob.
  const sections = raw.split(/\n(?=#{1,6}\s)/).map(s => s.trim()).filter(Boolean);
  const chunks = [];
  for (const section of sections) {
    const html = marked.parse(section);
    chunks.push(...chunkText(stripHtml(html), filePath));
  }
  // chunkText() numbers each section's chunks from 0, so ids collide across
  // sections within the same file — renumber globally before returning.
  return chunks.map((c, i) => ({ ...c, id: `${filePath}#${i}` }));
}
